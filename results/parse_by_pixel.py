# parse_grid_results_by_pixel.py
"""
Parses the final JSON results from a WE-PaDiM grid search run,
extracts key metrics and parameters, ranks the configurations based
primarily on AvgPixelAUC, and saves a summary CSV.
Optionally prints resource summary info.
"""
...
def parse_results(results_dir: str, model_name: str):
    ...
    """
    print(f"\n--- Parsing Results for Model: {model_name} (Ranking by Pixel AUC) ---")
    ...
    # --- MODIFIED SORTING ---
    # Sort primarily by Pixel AUC descending, then Image AUC descending, then PRO Score
    sort_columns = ['AvgPixelAUC', 'AvgImageAUC', 'AvgPROScore']
    ...
    # Save Summary CSV with a new name
    summary_csv_path = os.path.join(results_dir, f"{model_name}_grid_summary_ranked_by_pixel.csv")
    # --- END MODIFIED SAVE FILENAME ---
    try:
        df.to_csv(summary_csv_path, index=False, float_format='%.6f')
        print(f"\nRanked summary (by pixel AUC) saved to: {summary_csv_path}")
    except Exception as e:
        print(f"ERROR: Failed to save summary CSV: {e}")

    # --- Optionally Parse and Print Resource Summary (kept same as original) ---
    # ... (resource summary parsing code remains unchanged) ...
...
def main():
    parser = argparse.ArgumentParser(description='Parse WE-PaDiM Grid Search Results (Ranked by Pixel AUC)')
